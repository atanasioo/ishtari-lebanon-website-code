...
import WidgetsPreviewComponent from "@/components/widgetsComponents/widgetsPreviewComponent";
...
//   TEST
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    var obj = {
      view: testquery?.view || "web_desktop",
      limit: 10,
      page: page,
      widget_group_id: testquery?.widget_group_id || null,
      status:  testquery?.status || null,
      view_id: testquery?.view_id || null,
    };
    let link =
...
// console.log("obj",obj)
    ...
    return dataRef.current.map((widget, index) => {
      if (dataRef.current.length === index + 1) {
        return (
          ...
          >
            {" "}
            <WidgetsPreviewComponent widget={widget} bannerStats={bannerStats} />{" "}
          </div>
        );
      } else {
        return (
  ...
  return (
    <div>
      ...
      <div className={`overflow-hidden  ${mobileViews.includes(testquery.view) ? "mx-[30rem]": "container"}`}>
        {mobileViews.includes(testquery.view) && (
          <ScrollToTop
            smooth
