...
function WidgetsPreviewComponent({ widget, likedData, bannerStats }) {
  ...
  const source_type_id =
    ...
    typeof router.query.slug !== "undefined"
      ? router.query.slug[0].includes("p=") ||
        ...
        : router.query.slug[0]
      : "";
console.log("source_type_id",source_type_id)
  const types = {
    1: "product",
    2: "category",
    3: "manufacturer",
  ...
  function CustomSliderPrevArrows({ direction, onClick, style, className }) {
    return (
      <div
        className={`swiper-button-next ${mobileViews.includes(router.query.view) && "hidden"}`}
  ...
  function CustomSliderNextArrows({ direction, onClick, style, className }) {
    return (
      <div
        className={`swiper-button-prev ${mobileViews.includes(router.query.view)  && "hidden"}`}
  ...
  function CustomPrevArrows({ direction, onClick, style, className }) {
    return (
      <div
        ...
        onClick={onClick}
        className={`${mobileViews.includes(router.query.view) && "hidden"} mySwiper `}      
        >
    ...
    >
      ...
      {widget?.display === "slider" &&
        ...
        (widget.items.length > 1 ? (
          <div className=" mb-2">
            {!mobileViews.includes(router.query.view) ? (
                ...
                {widget.items.map((item, index) =>
                  item.mobile_type_id === "0" ? (
                    <div
                      data-index={index}
                      key={`sliderM` + index}
                      className="relative placeHolderSlideCSS w-full flex justify-center"
                    >
                      <Image
                        alt={item?.name}
                        src={host.host + "/image/" + item.image}
                        className="w-full"
                        height={mobileViews.includes(router.query.view) ? 2000 : widget.banner_height}
                        width={mobileViews.includes(router.query.view) ? 3000 :widget.banner_width}
      ...
      {widget?.display === "grid" &&
        ...
        widget.column_number < 7 && (
          <div className={`${mobileViews.includes(router.query.view) ? "flex flex-wrap justify-center":"flex -mx-4 flex-wrap justify-between"}`}>
            {widget.items.map((item) => {
                ...
                >
                  ...
                  >
                    <Image
                      alt={item?.name}
                      src={host.host + "/image/" + item.image}
                      width={mobileViews.includes(router.query.view) ? 2000 :widget.banner_width}
                      height={mobileViews.includes(router.query.view) ? 1000:widget.banner_height}
                      title={item?.name}
                      className={"w-full placeHolderSlideCSS"}
          ...
          {widget?.items?.length < 7 ? (
            <div className="mobile:flex">
              {!mobileViews.includes(router.query.view) ? (
                  ...
                  {widget.row_number === "2" ? (
                    ...
                    >
                      {widget.items?.map((item) => {
                        if (item.product_id) {
                          return (
                            <div className="pr-2 border" key={item.product_id}>
                              <SingleProduct
                                likedData={likedData}
                                item={item}
                                click={handleOnItemClick}
                ...
                // overflow carousel mobile
                <div className={`${mobileViews.includes(router.query.view) ? "block": "hidden"}`}>
                  <div className="flex overflow-x-auto space-x-2 ">
                    {widget?.display === "carousel" &&
                      ...
                      widget.items?.map((item) => {
                        if (item.product_id) {
                          return (
                            <div className="w-full bg-dbase" key={item.product_id}>
                              <SingleProduct
                                scroll={true}
                                item={item}
                              ></SingleProduct>
            ...
            <div className="w-full ">
              {widget.row_number === "2" ? (
                ...
                <div className="flex flex-col">
                  <div className={`${!mobileViews.includes(router.query.view) && "hidden"} snipex flex justify-between overflow-x-auto snap-mandatory  space-x-2 overflow-hidden`}>
                    {widget?.display === "carousel" &&
                      widget.type !== "text" &&
                      widget.items?.map((item) => {
                        if (item?.product_id)
                          return (
                            <div className="flex-grow bg-dTransparentWhite1" key={item.product_id}>
                               <SingleProductTest
                                scroll={true}
                                item={item}
                              ></SingleProductTest> 
              ...
              {width > 650 && !widget?.scrolling && (
                <div className={mobileViews.includes(router.query.view) ? "hidden" : "block"}>
                  {widget.row_number === "2" ? (
                    <Slider
                      {...twoRowsSettings}
                      beforeChange={handleBeforeChange}
                    ...
                    >
                      {widget.items?.map((item) => {
                        if (item.product_id && !mobileViews.includes(router.query.view)) {
                          return (
                          ...
                          return (
                            <div
                              className={`p-1 cursor-pointer hover:opacity-80 ${mobileViews.includes(router.query.view) ? "" :`w-1/${widget.column_number} md:w-1/${widget.column_number}`} `}
                              key={item.banner_image_id}
                            >
                              {/* here */}
                              <BannerLink
...
export default WidgetsPreviewComponent;
