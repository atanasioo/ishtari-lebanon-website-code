...
function ProductZoomModal(props) {
  ...
  const mobileSetting = {
    ...
    prevArrow: (
      <div>
        <BsChevronLeft className="w-8 h-8 -ml-2.5 text-darrowZoom" />
      </div>
    ), // or null
    nextArrow: (
      <div>
        <BsChevronRight className="w-8 h-8 ml-4 text-darrowZoom" />
      </div>
    ) // or null
  };
    ...
    <div className="fixed bg-white md:bg-dblackOverlay top-0 lef-0 right-0 bottom-0 w-full h-full z-30 overflow-hidden">
      <div className="modal_zoom_div relative z-40 h-full mx-auto text-center box-border">
        <div className="modal_zoom_div absolute w-full lg:w-11/12 m-auto h-full  z-50 bg-white top-0 left-0 right-0 bottom-0 lg:max-h-90%">
          <div
            className=" m-0 p-0 md:min-h-full modal_zoom_div md:h-full bg-dblackOverlay3 lg:bg-white"
            id="popup_modal"
          >
            <CgClose
            ...
            <div
              className={`flex flex-col justify-center ${
                width < 768 ? "items-center" : ""
              } md:justify-center modal_zoom_div  lg:h-unset mb-5`}
            >
