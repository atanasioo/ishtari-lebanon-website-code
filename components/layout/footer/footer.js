  ...
  return (
    <div className={`flex justify-between mt-5  `}>
          ...
          <div key={cat?.category_id} className="mt-2 pb-3">
            ...
            {cat?.data?.map((sub) => {
              return (
                <div
                  key={sub.category_id}
                  className="flex font-light loading-dtight py-1.5"
                >
      ...
      <div className="mt-3">
        <div>
          <div
            className=" uppercase  hover:text-dblue pr-bold text-df pb-3   pr-semibold loading-dsnug"
            dangerouslySetInnerHTML={{ __html: "TOP BRAND" }}
          />
        </div>
        {data?.brand?.map((brand) => {
          return (
            <div key={brand.id}     className="flex font-light loading-dtight py-1.5"
            >
              <Link
                className="text-d14 hover:text-dblue"
                href={`${
